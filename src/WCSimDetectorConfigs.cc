...
void WCSimDetectorConstruction::SetNuPrismGeometry()
{
  ...
  WCBorderPMTOffset     = 0.05*m; // PMT offset in border cells. Also makes distance between PMTs shorter.
	
  WCBarrelPMTOffset     = WCPMTRadius;
  WCPMTperCellHorizontal = 3.0; // These ID values chosen to allow for proper OD coverage
  WCPMTperCellVertical   = 4.0; // These ID values chosen to allow for proper OD coverage
  ...
  // OD Coverage on caps //
  WCPMTODPercentCoverage   = 1.0; // Actually works out to 0.8788 on caps and 0.93 on barrel with current ID config
